...
('Oretha Menter', 'oretha_menter@abc.com','172307504')
...
--Q1--Begin
select state from DIM_LOCATION as S inner join FACT_TRANSACTIONS as F
on  F.IDlocation=S.IDlocation
where YEAR (F.[Date])>=2005
group by S.State

--Q1-- End
--Q2--Begin
select [state] from (select top 1 L.[state],sum (F.quantity) as [Total_qty] from FACT_TRANSACTIONS as F 
...
--Q3--Begin
select L.ZipCode,Mo.IDModel,L.[state],count(F.IDcustomer) as [Total_Transactions] from  FACT_TRANSACTIONS as F
inner join DIM_LOCATION as L on F.IDLocation=L.IDLocation
inner join DIM_MODEL as Mo on Mo.IDModel=F.IDModel
group by L.State,L.ZipCode,Mo.IDModel
--Q3--End
--Q4--Begin
select M.Manufacturer_Name,Mo.model_name,Mo.idmodel,Mo.Unit_price from DIM_MODEL as Mo
inner join DIM_MANUFACTURER as M on M.IDManufacturer=Mo.IDManufacturer
where Unit_price=(select min(unit_price) from DIM_MODEL)
--Q4--End
--Q5--Begin
select top 5 M.Manufacturer_Name,M.IDmanufacturer,sum(F.Quantity)as tot_qty,Avg(F.totalPrice)as [avgprice] from FACT_TRANSACTIONS as F
inner join DIM_MODEL as MO on mo.IDModel=F.IDModel
inner join Dim_manufacturer as M on M.IDManufacturer=Mo.IDManufacturer
...
--Q6--Begin
select  Mo.Model_name,mo.IDmanufacturer,Avg(F.totalprice)as[Avg] from Fact_Transactions as F
inner join DIM_model as MO on Mo.IDmodel=f.IDmodel
group by Mo.model_name,Mo.IDmanufacturer


create view [table 1] as
select top 5 M.Manufacturer_Name,M.IDmanufacturer,sum(F.Quantity)as tot_qty,Avg(F.totalPrice)as [avgprice] from FACT_TRANSACTIONS as F
inner join DIM_MODEL as MO on mo.IDModel=F.IDModel
inner join Dim_manufacturer as M on M.IDManufacturer=Mo.IDManufacturer
group by M.Manufacturer_Name,M.IDManufacturer
...
create view [Table 3] as
select Mo.Model_name,mo.IDmanufacturer,Avg(F.totalprice)as[Avg] from Fact_Transactions as F
inner join DIM_model as MO on Mo.IDmodel=f.IDmodel
group by Mo.model_name,Mo.IDmanufacturer


select model_name,[Avg] as Average_price from  [table 3] as T3 
inner join [Table 1] as T1 on T3.IDmanufacturer=T1.IDmanufacturer;

--Q6--End

--Q7--Begin
select C.Customer_Name,C.IDcustomer,AVG(F.totalprice) as [average amount] from DIM_CUSTOMER as C
...
create view [a1] as
select top 5 mo.Model_Name,Mo.IDModel,SUM(F.Quantity) as totalQty from FACT_TRANSACTIONS as F
inner join DIM_MODEL as Mo on Mo.IDModel=F.IDModel
...
inner join [a3]
on a2.idmodel=a3.IDmodel;
--Q7--End

--Q8--Begin
select *from(select row_number() over(order by sum(F.totalprice)desc)as R, M.Manufacturer_Name,sum(F.Totalprice) as total_amt from  FACT_TRANSACTIONS as F
inner join DIM_MODEL as Mo on Mo.IDModel=F.IDmodel
inner join DIM_MANUFACTURER as M on M.IDManufacturer=Mo.IDManufacturer
where year (F.[Date])=2009
...
create view [t1] as
select M.Manufacturer_Name,sum(F.TotalPrice) as total_amt from FACT_TRANSACTIONS as F
inner join DIM_MODEL as Mo on Mo.IDModel=F.IDmodel
inner join DIM_MANUFACTURER as M on M.IDManufacturer=Mo.IDManufacturer
where YEAR (F.[Date])=2010
...
select manufacturer_name  from [t1] 
except
select manufacturer_name  from [t2] 
...
--Q10--Begin
select TBl1.IDCustomer,TBl1.customer_name,TBl1.[year],TBl1.Avg_spend,TBl1.Avg_Qty , 
case when TBl2.[year] is not null then ((TBl1.Avg_spend-TBl2.Avg_spend) /TBl2.Avg_spend*100)
else null
end as 'Percentage of change in their spend' from
...
where C.IDCustomer in (select top 100 C.IDCustomer from DIM_CUSTOMER as C
left join FACT_TRANSACTIONS as F on F.IDcustomer=C.IDcustomer
